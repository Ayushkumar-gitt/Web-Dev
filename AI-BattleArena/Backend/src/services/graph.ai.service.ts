...
const judgeNode: GraphNode<typeof State> = async (state) => {
    ...
    const judge = createAgent({
        ...
        systemPrompt: "You are a judge tasked with evaluating two solutions generated by different ai models. please provide a score out of 10 for each solution"
    })

    const judge_response = judge.invoke({
        messages: [
            new HumanMessage(`Solution 1: ${solution_1}\nSolution 2: ${solution_2}\nPlease provide a score out of 10 for each solution. Make sure both are not same`)
        ]
    })
    const { solution_1_score, solution_2_score } = (await judge_response).structuredResponse;
    return {
        judge_recomendation: {
            solution_1_score,
            solution_2_score,
            winner: solution_1_score > solution_2_score ? "solution 1" : "solution 2"
        }
    }
}
